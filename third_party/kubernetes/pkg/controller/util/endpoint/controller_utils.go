...
// Package endpoitn is copied from k8s.io/kubernetes/pkg/controller/util/endpoint
package endpoint
...
func GetPodServiceMemberships(serviceLister v1listers.ServiceLister, pod *v1.Pod) (sets.String, error) {
	...
	for i := range services {
		key, err := cache.DeletionHandlingMetaNamespaceKeyFunc(services[i])
		if err != nil {
			return nil, err
		}
		set.Insert(key)
...
// NOTE(bo.liub): DetermineNeededServiceUpdates change to public
