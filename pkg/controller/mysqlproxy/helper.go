...
var (
	...
	mysqlProxyGVK  = v1alpha1.SchemeGroupVersion.WithKind("MysqlProxy")
)
...
func (c *Controller) createStatefulSet(p *v1alpha1.MysqlProxy) (*appsv1.StatefulSet, error) {
	...
	sts.OwnerReferences = append(sts.OwnerReferences, *metav1.NewControllerRef(p, mysqlProxyGVK))

	return c.kubeClient.AppsV1().StatefulSets(sts.Namespace).Create(context.TODO(),&sts,metav1.CreateOptions{})
}

func (c *Controller) createService(p *v1alpha1.MysqlProxy) (*corev1.Service, error) {
	svc := corev1.Service{}
	...
	svc.OwnerReferences = append(svc.OwnerReferences, *metav1.NewControllerRef(p, mysqlProxyGVK))

	return c.kubeClient.CoreV1().Services(svc.Namespace).Create(context.TODO(),&svc,metav1.CreateOptions{})
}

func (c *Controller) createConfigMap(p *v1alpha1.MysqlProxy) (*corev1.ConfigMap, error) {
	cm := corev1.ConfigMap{}
	d := ConfigMapData{
		MysqlProxy: *p,
		Extra: ConfigMapExtraData{
			Secret: c.mysqlSecret,
		},
	}
	...
	cm.OwnerReferences = append(cm.OwnerReferences, *metav1.NewControllerRef(p, mysqlProxyGVK))

	return c.kubeClient.CoreV1().ConfigMaps(cm.Namespace).Create(context.TODO(),&cm,metav1.CreateOptions{})
}

func (c *Controller) tryUpdateStatefulSet(sts *appsv1.StatefulSet, p *v1alpha1.MysqlProxy) (*appsv1.StatefulSet, error) {
	...
	stsNew.OwnerReferences = append(stsNew.OwnerReferences, *metav1.NewControllerRef(p, mysqlProxyGVK))
	...
	klog.Infof("Update statefulset of mysql proxy %v", p.Name)
...
func (c *Controller) tryUpdateService(svc *corev1.Service, p *v1alpha1.MysqlProxy) (*corev1.Service, error) {
	...
	svcNew.OwnerReferences = append(svcNew.OwnerReferences, *metav1.NewControllerRef(p, mysqlProxyGVK))
	...
	klog.Infof("Update service of mysql proxy %v", p.Name)
...
func (c *Controller) tryUpdateConfigMap(cm *corev1.ConfigMap, p *v1alpha1.MysqlProxy) (*corev1.ConfigMap, error) {
	...
	cmNew.OwnerReferences = append(cmNew.OwnerReferences, *metav1.NewControllerRef(p, mysqlProxyGVK))
	...
	klog.Infof("Update configmap of mysql proxy %v", p.Name)
...
func (c *Controller) getMysqlProxyFromPod(pod *corev1.Pod) *v1alpha1.MysqlProxy {
	...
	if ref.Kind != mysqlProxyGVK.Kind {
		return nil
	}

	mp, err := c.mysqlProxyLister.MysqlProxies(pod.Namespace).Get(ref.Name)
	if err != nil || mp.UID != ref.UID {
		klog.V(4).Infof("Cannot get mysql proxy %q for pod %q: %v", ref.Name, pod.Name, err)
		return nil
	}
...
func (c *Controller) getMysqlProxyFromEndpoints(ep *corev1.Endpoints) *v1alpha1.MysqlProxy {
	mp, err := c.mysqlProxyLister.MysqlProxies(ep.Namespace).Get(ep.Name)
	if err != nil {
		klog.V(4).Infof("Cannot get mysql proxy %q: %v", ep.Name, err)
		return nil
	}
	return mp
}
