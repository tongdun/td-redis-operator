...
type ControllerOptions struct {
	...
	// ServiceTemplate defines template of redis standalone service
	...
	// RedisStandaloneInfromer defines informer of redis standalone
	RedisStandaloneInformer cacheinformers.RedisStandaloneInformer
...
type Controller struct {
	...
	redisStandaloneLister cachelisters.RedisStandaloneLister
...
func NewController(opt *ControllerOptions) *Controller {
	...
	c := &Controller{
		...
		informersSynced: []cache.InformerSynced{
			...
			opt.RedisStandaloneInformer.Informer().HasSynced,
		},
		queue:                 workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "redisstandalone"),
		redisStandaloneLister: opt.RedisStandaloneInformer.Lister(),
	...
	*/
	opt.RedisStandaloneInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc:    c.addRedisStandalone,
		UpdateFunc: c.updateRedisStandalone,
		DeleteFunc: c.deleteRedisStandalone,
	})

	return c
}

func (c *Controller) addRedisStandalone(obj interface{}) {
...
func (c *Controller) updateRedisStandalone(old, cur interface{}) {
	c.addRedisStandalone(cur)
}

func (c *Controller) deleteRedisStandalone(obj interface{}) {
	c.addRedisStandalone(obj)
}

func (c *Controller) addEndpoints(obj interface{}) {
	ep := obj.(*corev1.Endpoints)
	mp := c.getRedisStandaloneFromEndpoints(ep)
	if mp == nil {
		return
	}
	c.addRedisStandalone(mp)
}
...
func (c *Controller) addPod(obj interface{}) {
	pod := obj.(*corev1.Pod)
	mp := c.getRedisStandaloneFromPod(pod)
	if mp == nil {
		return
	}
	c.addRedisStandalone(mp)
...
func (c *Controller) Run(workers int, stopCh <-chan struct{}) {
	...
	klog.Infof("Starting redis standalone controller")
	defer klog.Infof("Shutting down redis standalone controller")

	if !cache.WaitForCacheSync(stopCh, c.informersSynced...) {
		utilruntime.HandleError(fmt.Errorf("unable to sync caches for redis standalone controller"))
		return
	}
	...
	for i := 0; i < workers; i++ {
		controller.WaitUntil("redisstandalone", c.reconcilerFactory(c.queue, c.syncRedisStandalone), stopCh)
	}

	klog.Infof("redis standalone controller is working")
	<-stopCh
}
