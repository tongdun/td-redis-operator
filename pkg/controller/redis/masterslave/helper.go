...
var (
	...
	redisStandbyGVK = v1alpha1.SchemeGroupVersion.WithKind("RedisStandby")
)

// ConfigMapData used to render configmap
type ConfigMapData struct {
	v1alpha1.RedisStandby
...
func (c *Controller) createStatefulSet(p *v1alpha1.RedisStandby) (*appsv1.StatefulSet, error) {
	...
	sts.OwnerReferences = append(sts.OwnerReferences, *metav1.NewControllerRef(p, redisStandbyGVK))

	return c.kubeClient.AppsV1().StatefulSets(sts.Namespace).Create(context.TODO(), &sts, metav1.CreateOptions{})
}

func (c *Controller) createSentiStatefulSet(p *v1alpha1.RedisStandby) (*appsv1.StatefulSet, error) {
	sts := appsv1.StatefulSet{}
	if err := c.sentiStatefulSetTemp.Execute(p, &sts); err != nil {
		klog.Errorf("can't render sentinel statefulset template for %v", p.Name)
		return nil, err
	}
...
func (c *Controller) createService(p *v1alpha1.RedisStandby) (*corev1.Service, error) {
	svc := corev1.Service{}
	...
	svc.OwnerReferences = append(svc.OwnerReferences, *metav1.NewControllerRef(p, redisStandbyGVK))
	return c.kubeClient.CoreV1().Services(svc.Namespace).Create(context.TODO(), &svc, metav1.CreateOptions{})
}

func (c *Controller) createSentiService(p *v1alpha1.RedisStandby) (*corev1.Service, error) {
	svc := corev1.Service{}
	if err := c.sentiServiceTemp.Execute(p, &svc); err != nil {
		klog.Errorf("can't render sentinel service template for %v", p.Name)
		return nil, err
	}
...
func (c *Controller) createConfigMap(p *v1alpha1.RedisStandby) (*corev1.ConfigMap, error) {
	cm := corev1.ConfigMap{}
	d := ConfigMapData{
		RedisStandby: *p,
		/*Extra: ConfigMapExtraData{
	...
	cm.OwnerReferences = append(cm.OwnerReferences, *metav1.NewControllerRef(p, redisStandbyGVK))

	return c.kubeClient.CoreV1().ConfigMaps(cm.Namespace).Create(context.TODO(), &cm, metav1.CreateOptions{})
}

func (c *Controller) tryUpdateStatefulSet(sts *appsv1.StatefulSet, p *v1alpha1.RedisStandby) (*appsv1.StatefulSet, error) {
	...
	stsNew.OwnerReferences = append(stsNew.OwnerReferences, *metav1.NewControllerRef(p, redisStandbyGVK))
...
func (c *Controller) tryUpdateSentiStatefulSet(sts *appsv1.StatefulSet, p *v1alpha1.RedisStandby) (*appsv1.StatefulSet, error) {
	stsNew := appsv1.StatefulSet{}
	if err := c.sentiStatefulSetTemp.Execute(p, &stsNew); err != nil {
		klog.Errorf("can't render sentinel statefulset template for %v", p.Name)
		return nil, err
	}
	...
	klog.Infof("Update sentinel statefulset of redis standby %v", p.Name)
...
func (c *Controller) tryUpdateService(svc *corev1.Service, p *v1alpha1.RedisStandby) (*corev1.Service, error) {
	...
	svcNew.OwnerReferences = append(svcNew.OwnerReferences, *metav1.NewControllerRef(p, redisStandbyGVK))
	...
	klog.Infof("Update service of redis standby %v", p.Name)
...
func (c *Controller) tryUpdateSentiService(svc *corev1.Service, p *v1alpha1.RedisStandby) (*corev1.Service, error) {
	svcNew := corev1.Service{}
	if err := c.sentiServiceTemp.Execute(p, &svcNew); err != nil {
		klog.Errorf("can't render sentinel service template for %v", p.Name)
		return nil, err
	}
	...
	klog.Infof("Update service of sentinel service %v", p.Name)
...
func (c *Controller) tryUpdateConfigMap(cm *corev1.ConfigMap, p *v1alpha1.RedisStandby) (*corev1.ConfigMap, error) {
	...
	cmNew.OwnerReferences = append(cmNew.OwnerReferences, *metav1.NewControllerRef(p, redisStandbyGVK))
...
func (c *Controller) getRedisMsFromPod(pod *corev1.Pod) *v1alpha1.RedisStandby {
	...
	if ref.Kind != redisStandbyGVK.Kind {
		return nil
	}

	mp, err := c.redisStandbyLister.RedisStandbies(pod.Namespace).Get(ref.Name)
...
func (c *Controller) getRedisMsFromEndpoints(ep *corev1.Endpoints) *v1alpha1.RedisStandby {
	mp, err := c.redisStandbyLister.RedisStandbies(ep.Namespace).Get(ep.Name)
	if err != nil {
